...
### Fitting a model  
The first step in any analysis is to import the data. In order to fit a model using the function `tor_fit()`, we need a set of measurements of metabolic rate $M$ and of the ambient temperature $T_a$ at which they were made. These values can be vectors of the same length or two columns of a data frame. In the chosen example on the Tasmanian pygmy possum, *Cercartetus lepidus*, e have 103 metabolic rate values at various $T_a$ that were digitized from a published figure (Geiser, 1987).
...
model <-tor_fit(Ta = test_data2$Ta, 
                ...
                fitting_options = list(ni = 500,
                                            ...
                                            parallel =FALSE))
```
Note that for the purpose of this vignette we only run two chains with 1’000 iterations and 800 burned-in. The results have to be considered carefully (i.e. Checking the adequacy of the model, below). Default settings are more suitable but require more time for the model to run.

The output of the `tor_fit()` function is a list containing information on the posterior distributions of the investigated parameters as well as on the convergence of the chains and on the prior-posterior distributions overlap of some parameters. These information can also be called by the function `tor_summarise()`. The rest of the package offers several functions to make more sense of the model output, to check its consistency and to represent it graphically. Researchers familiar with `jagsUI` can develop their analysis from that point.
...
The identifiability of the parameters can be checked by comparing their prior and the posterior distributions. If the overlap between these is greater than 85%, the parameter should be considered as not identifiable (Fasel et al. in prep.) and a warning will be sent to the user. Parameter identifiability is provided for $T_{lc}$, $M_r$, and $T_{be}$, $TMR$ and $T_{bt}$. The ovelap is also given by the `tor_summarise()` function. Let’s continue with our analysis of *Cercartetus lepidus*:
...
To look at the classification of the data the corresponding predicted values we use the function `tor_assign()`,  which will assign the measured metabolic values to either torpor or euthermia  and returns a dataframe with the measured $M$, the measured $T_a$, the predicted $M$ and the assignment.
...
### Prediction 
The `tor_predict()`  function is slightly different as it takes a vector of $T_a$ as input and returns the predicted  $M$ both in torpor and in euthermia and the 95% credible interval. For example, the predicted $M$ at $T_a$ 22°C can be found:
...
Finally a built-in function allows to plot the results. The user can modify the labels with `xlab` and `ylab` and the colors with `col_torp`, `col_eut` and `col_Mtnz` and can save the plot using the arguments `pdf = TRUE`. The plot_type argument also allows to choose between `base` (default) and `ggplot`.
