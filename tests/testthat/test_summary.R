 ...
 test_that("tor_ppo return a data.frame of 2 rows and 2 columns", {
   set.seed(1)
   expect_warning(mod <- tor_fit(M = test_data$VO2, Ta = test_data$Ta, fitting_options = list(ni = 5000, nb = 3000, nc = 1)))
   set.seed(1)
 ...
 test_that("tor_ppo and tor_summarise return an error if the input is not a jagsUI object fitted with tor_fit", {
   ...
   expect_error(tor_ppo(mod))
   expect_error(tor_summarise(mod))
})
#
test_that("tor_ppo and tor_summarise return a warning if tlc_overlap, Tt_overlap and Betat_overla >= 0.3)", {
   set.seed(1)
   expect_warning(mod <- tor_fit(M = test_data$VO2, Ta = test_data$Ta, fitting_options = list(ni = 5000, nb = 3000, nc = 1)))
 ...
 test_that("get_parameters return a data.frame of 15 rows and 6 col", {
   set.seed(1)
   expect_warning(mod <- tor_fit(M = test_data$VO2, Ta = test_data$Ta, fitting_options = list(ni = 5000, nb = 3000, nc = 1)))
   test <- get_parameters(mod)
   expect_equal(nrow(test), 15)
