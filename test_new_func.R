...
## rm na from data and reshuffle order.
data <- data[!is.na(data$VO2ms)&!is.na(data$Ta),]
set.seed(666)

  ord <- order(rnorm(length(data$VO2ms),0,1))
  data <- data[ord,]

  ########First step: Find the BMR and get the lowest possible tlc value.
step_2 <- function(Y, Ta, fitting_options = list(ni = 500000,
                                                    nt = 10,
                                                    nb = 300000,
                                                    nc = 3)) {
  out_1 <- step_1(Y, Ta)
  ...
  out1 <- jagsUI ::jags(data = win.data,
...
out_2 <- step_2(Ta = test_data2$Ta, Y = test_data2$VO2ms)



  ########Third step: Assign to torpor, euthermy or TNZ.

step_3 <- function(Y, Ta, fitting_options = list(ni = 500000,
                                                   nt = 10,
                                                   nb = 300000,
                                                   nc = 3)){
  ...
  data <- cbind(Y, Ta)[!is.na(Y) & !is.na(Ta), ]

  Y <- data[, "Y"]
  Ta <- data[, "Ta"]

  ## run step 1 and 2 to get bmr and tlc
  out_2 <- step_2(Ta = Ta, Y = Y)
  ...
  out_2 <- step_3(Ta = data$Ta, Y = data$VO2ms)


step_2_bis <- function(mod){
...
step_4 <- function(out_2, tt,  fitting_options = list(ni = 500000,
                                                      nt = 10,
                                                      nb = 300000,
                                                      nc = 3)) {
  Y <- out_2$data$Y
